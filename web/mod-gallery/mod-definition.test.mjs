...
test('demo-combo GalleryパッケージはFirmwareサンプルと同じ実行ソースを公開する', () => {
  const firmware = new URL('../../firmware/mods/examples/demo_combo/', import.meta.url)
  const gallery = new URL('./samples/demo-combo/mod/', import.meta.url)
  for (const filename of ['manifest.json', 'mod.js']) {
    assert.equal(
      readFileSync(new URL(filename, gallery), 'utf8'),
      readFileSync(new URL(filename, firmware), 'utf8'),
